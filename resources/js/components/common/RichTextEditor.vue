...
const props = defineProps({
  ...
  // Upload configuration - requires callback to handle upload
  uploadEnabled: {
    type: Boolean,
    default: false
  },
  // Callback function for uploading images
  // Should return a Promise that resolves to { url: string }
  onImageUpload: {
    type: Function,
    default: null
  }
...
const handleFileInputChange = async (event) => {
  ...
  if (!file.type.startsWith('image/')) {
    emit('image-uploaded', { error: 'Only image files are allowed' })
    return
  }
  ...
  if (file.size > maxSize) {
    emit('image-uploaded', { error: 'Image size must be less than 10MB' })
    return
  }

  if (!props.onImageUpload) {
    emit('image-uploaded', { error: 'No upload handler configured' })
    return
  }
  ...
  try {
    ...
    if (response && response.url) {
      ...
      emit('image-uploaded', { success: true, url: response.url })
    }
  } catch (error) {
    emit('image-uploaded', { error: error.message || 'Failed to upload image' })
  } finally {
